...
for stt in state:
	...
	driver.get('https://www.healthgrades.com/diet-nutrition-directory/'+stt)
	driver.implicitly_wait(20)
	time.sleep(3)
			...
			try:
				...
				for l in lists:
					if 'card-name' in l.get_attribute('innerHTML'):
						f = open("healthgrades-dietitian-list.txt", "r")
			...
			try:
				...
				for n in name.replace('\'','').split(' '):
					if not n.isupper():
						new_name.append(n.lower().split(',')[0])
				new_name = ' '.join(new_name)
				with open("healthgrades-dietitian-list.txt", "a") as f:
					f.write(new_name+'\n')
			except Exception as e:
				pass
